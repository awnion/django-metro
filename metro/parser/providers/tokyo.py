...
class DataProvider(BaseEnDataProvider):
    metro_lines_src = f"{BaseEnDataProvider.base_en_url}/wiki/Tokyo_subway"
    lines_buf = []
    ...
    def download_stations(self):
        ...
        for line in self.lines_buf:
            # create line
            line_inst = self.get_or_create_line(line["number"], line["title"], line["color"])
            print(f"Download stations for line number {line['number']}")
            # download stations
            ...
            for row in table.find_all("tr", recursive=False):
                ...
                if tds and len(tds) > 1:
                    ...
                    self.get_or_create_station(line_inst, self.get_el_text(tds[index]))
