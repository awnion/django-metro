...
class BaseDataProvider:
    ...
    def __init__(self, station_model=None, line_model=None):
        if station_model and line_model:
            ...
            raise AttributeError("You need to provide base station and line models")
    ...
    def prep_color(self, el):
        try:
            ...
            return element.replace(self.bg_word, "").replace(";", "").strip()
        except KeyError:
            return ""
...
class BaseRuDataProvider(BaseDataProvider):
    ...
    def prep_title(self, el):
        return self.translit_if_needed(
            self.clean_title_re.sub(
                "", self.without_brackets_re.sub("", self.get_el_text(el).replace(self.line_word, "").strip())
            )
        )
    ...
    def parse_usual_big_table(self, station_td_count=None, table_number=None):
        ...
        table = body.find_all("table")[table_number] if table_number else body.find("table")
        ...
        for tr in table.find_all("tr", recursive=False):
            ...
            if td:
                ...
                if self.line_word in td.get_text():
                    ...
                    if station_td_count:
                        count = len(tr.find_all("td", recursive=False))
...
class BaseEnDataProvider(BaseDataProvider):
    ...
    def prep_title(self, el):
        return self.without_brackets_re.sub("", self.without_sbrackets_re.sub("", self.get_el_text(el).strip()))
